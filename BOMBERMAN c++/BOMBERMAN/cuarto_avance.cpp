...
void BomberMan::Mover(){
	...
	if (_kbhit()){
		...
		if((tecla == DERECHA && Mapa[y][x+1] != '°')&&(tecla == DERECHA && Mapa02[y][x+1] != '°')){
			//if(Mapa[y][x+1]=='@'){ energy--;}
			 
			x++;
		}
		if((tecla == IZQUIERDA && Mapa[y][x-1] != '°')&&(tecla == IZQUIERDA && Mapa02[y][x-1] != '°')){
			//if(Mapa[y][x-1]=='@'){ energy--;}
			
			x--;
		}
		if((tecla == ARRIBA && Mapa[y-1][x] != '°')&&(tecla == ARRIBA && Mapa02[y-1][x] != '°')){
			//if(Mapa[y-1][x]=='@'){ energy--;}
			
			y--;
		}
		if((tecla == ABAJO && Mapa[y+1][x] != '°')&&(tecla == ABAJO && Mapa02[y+1][x] != '°')){
			//if(Mapa[y+1][x]=='@'){ energy--;}
...
void DeadShots::Mover(){
			...
			if(tecla == DERECHA && Mapa02[y][x+1] != '°'){
				if(Mapa02[y][x+1]==char(155)){ vida = false;}
				x++;
			}
			if(tecla == IZQUIERDA && Mapa02[y][x-1] != '°'){
				if(Mapa02[y][x-1]==char(155)){ vida = false;}
				x--;
			}
			if(tecla == ARRIBA && Mapa02[y-1][x] != '°'){
				if(Mapa02[y-1][x]==char(155)){ vida = false;}
				y--;
			}
			if(tecla == ABAJO && Mapa02[y+1][x] != '°'){
				if(Mapa02[y+1][x]==char(155)){ vida = false;}
				y++;
			}
...
int main(){
    ...
    string menu_win[3]={    "          ¡BIGBOSS ha ganado!                ",
    						"                                             ",
	 						"    Presiona X para continuar:               "};
		...
		if(menu_dos.opcion == "A" || menu_dos.opcion == "a"){
			...
			if (submenu_uno.opcion == "X" || submenu_uno.opcion == "x") {
				...
				while(facil==true){
					...
					if(distancia<BomberMan>(BM,BB)<=6){
						BM.energy--; 
					}
					Imprimir(Mapa);
